...
function sendViaProxy(to, subject, html) {
    ...
    .catch(function(err) {
        console.error('❌ Email proxy error:', err.message || err);
        return { ok: false };
    });
}
...
function sendQuoteEmailToCustomer(data) {
    ...
    var html =
        ...
        '<td><span style="font-size:.72rem;color:#5a8a96;display:block">Quote Reference</span><strong style="color:#0d4a5c">' + (data.quoteRef || '—') + '</strong></td>' +
        ...
        '<tr><td style="padding:8px 12px;background:#f0f7f9;font-size:.82rem;color:#5a8a96;width:35%">Name</td><td style="padding:8px 12px;font-size:.88rem;color:#0d4a5c;font-weight:600">' + data.customer.name + '</td></tr>' +
        ...
        (data.onceOffTotal > 0 ?
            ...
            (data.techIncluded && data.techHours > 0 ? '<tr><td style="padding:10px 14px;border-bottom:1px solid #e4f1f5;font-size:.85rem;color:#2a5f70">Technician Setup (' + data.techHours + ' hrs x R450)</td><td style="padding:10px 14px;border-bottom:1px solid #e4f1f5;font-size:.85rem;color:#0d4a5c;text-align:right;font-weight:700">R' + (data.techTotal || 0).toLocaleString() + '.00</td></tr>' : '') +
            (data.trainingIncluded ? '<tr><td style="padding:10px 14px;border-bottom:1px solid #e4f1f5;font-size:.85rem;color:#2a5f70">Staff Training</td><td style="padding:10px 14px;border-bottom:1px solid #e4f1f5;font-size:.85rem;color:#0d4a5c;text-align:right;font-weight:700">R' + (data.trainingTotal || 0).toLocaleString() + '.00</td></tr>' : '') +
        ...
        '<p style="font-size:.75rem;color:#8aabb5">StockAI-Pro • 079 044 0508 • support@stockai-pro.co.za • www.stockai-pro.co.za</p>' +
...
function sendQuoteNotificationToAdmin(data) {
    ...
    var html =
        ...
        '</div></div>';
    return sendViaProxy(ADMIN_EMAIL, 'New Quote: ' + (data.customer ? data.customer.business : 'Unknown') + ' — R' + (data.monthlyTotal || 0).toLocaleString() + '/mo', html);
}
...
function sendBookingConfirmationToCustomer(booking) {
    ...
    var html =
        ...
        '<div style="font-size:.78rem;color:#047857;margin-top:4px">Need to reschedule? Contact us 24 hours in advance.</div>' +
        '</div>' +
        '<p style="font-size:.75rem;color:#8aabb5;text-align:center">StockAI-Pro • 079 044 0508</p>' +
        '</div></div>';
    return sendViaProxy(booking.customer.email, 'Presentation Confirmed — ' + dateFormatted + ' — StockAI-Pro', html);
}
...
function sendBookingEmailToAdmin(booking) {
    ...
    var html =
        ...
        '</div>' +
        '<div style="background:#fff;padding:28px;border:1px solid #e4f1f5;border-top:none;border-radius:0 0 12px 12px">' +
        '<p style="color:#5a8a96">A new presentation has been booked. Add to your calendar!</p>' +
        ...
        '</table>' +
        '<div style="background:#fff8e1;border-left:3px solid #f59e0b;padding:14px;border-radius:8px">' +
        '<p style="color:#5a4a17;font-size:.85rem;margin:0">Add to calendar: ' + dateFormatted + ' — ' + booking.slotLabel + '</p>' +
        '</div>' +
        '<p style="font-size:.75rem;color:#8aabb5;text-align:center;margin-top:20px">StockAI-Pro</p>' +
        '</div></div>';
    return sendViaProxy(ADMIN_EMAIL, 'New Booking: ' + booking.customer.business + ' — ' + dateFormatted, html);
}
...
function sendDailyQuoteAnalysis() {
    ...
    var html =
        ...
        '<div style="flex:1;min-width:130px;background:#e4f1f5;border-radius:10px;padding:16px;text-align:center"><div style="font-size:2rem;font-weight:900;color:#1a8ba8">' + todayQuotes.length + '</div><div style="font-size:.72rem;color:#5a8a96">Quotes Today</div></div>' +
        '<div style="flex:1;min-width:130px;background:#d1fae5;border-radius:10px;padding:16px;text-align:center"><div style="font-size:1.2rem;font-weight:900;color:#065f46">R' + totalMonthly.toLocaleString() + '</div><div style="font-size:.72rem;color:#047857">Monthly Value</div></div>' +
        '<div style="flex:1;min-width:130px;background:#fef3c7;border-radius:10px;padding:16px;text-align:center"><div style="font-size:1.2rem;font-weight:900;color:#92400e">R' + totalOnceOff.toLocaleString() + '</div><div style="font-size:.72rem;color:#a16207">Once-off Value</div></div>' +
        '</div>' +
        ...
        '<p style="font-size:.75rem;color:#8aabb5;text-align:center;margin-top:20px">StockAI-Pro — www.stockai-pro.co.za</p>' +
        '</div></div>';
    return sendViaProxy(ADMIN_EMAIL, 'Daily Quotes: ' + todayQuotes.length + ' — R' + totalMonthly.toLocaleString() + '/mo potential', html);
}
...
console.log('✅ email-addon.js loaded');
