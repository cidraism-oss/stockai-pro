...
// Sends emails via Firebase Cloud Function proxy
// This bypasses the CORS restriction from the browser
// ============================================================
...
function sendViaProxy(to, subject, html) {
    ...
    return fetch(FIREBASE_EMAIL_URL, {
        method: 'POST',
    ...
    var html =
        ...
        (data.onceOffTotal > 0 ?
            ...
            (data.techIncluded && data.techHours > 0 ?
                '<tr><td style="padding:10px 14px;border-bottom:1px solid #e4f1f5;font-size:.85rem;color:#2a5f70">🔧 Technician Setup (' + data.techHours + ' hrs × R450)</td>' +
        ...
        '<a href="https://www.stockai-pro.co.za/signup.html" style="display:inline-block;padding:14px 32px;background:#f59e0b;color:#fff;text-decoration:none;border-radius:8px;font-weight:700;font-size:.95rem">🚀 Get Started Now</a>' +
        '</div>' +
...
function sendQuoteNotificationToAdmin(data) {
    ...
    var html =
        ...
        '<tr style="background:#e4f1f5"><td style="padding:12px;font-weight:900;color:#0d4a5c;font-size:.95rem">Monthly Total</td><td style="padding:12px;font-weight:900;color:#1a8ba8;font-size:1rem">R' + (data.monthlyTotal || 0).toLocaleString() + '.00/mo</td></tr>' +
        '<tr style="background:#fef3c7"><td style="padding:12px;font-weight:900;color:#0d4a5c;font-size:.95rem">Once-off Total</td><td style="padding:12px;font-weight:900;color:#d97706;font-size:1rem">R' + (data.onceOffTotal || 0).toLocaleString() + '.00</td></tr>' +
        '</table>' +
...
function sendBookingConfirmationToCustomer(booking) {
    ...
    var html =
        ...
        '</div>' +
        '<p style="font-size:.75rem;color:#8aabb5;text-align:center;margin-top:20px">© 2025 StockAI-Pro • 079 044 0508</p>' +
        '</div></div>';
...
function sendBookingEmailToAdmin(booking) {
    ...
    var html =
        ...
        '<tr><td style="padding:8px 12px;background:#f0f7f9;font-size:.82rem;color:#5a8a96">Phone</td><td style="padding:8px 12px;font-size:.88rem;color:#0d4a5c;font-weight:600">' + booking.customer.phone + '</td></tr>' +
        '<tr><td style="padding:8px 12px;background:#f0f7f9;font-size:.82rem;color:#5a8a96">Email</td><td style="padding:8px 12px;font-size:.88rem;color:#0d4a5c;font-weight:600">' + booking.customer.email + '</td></tr>' +
        '<tr><td style="padding:8px 12px;background:#f0f7f9;font-size:.82rem;color:#5a8a96">Booked At</td><td style="padding:8px 12px;font-size:.88rem;color:#0d4a5c;font-weight:600">' + new Date(booking.timestamp).toLocaleString('en-ZA') + '</td></tr>' +
        '</table>' +
...
function sendDailyQuoteAnalysis() {
    ...
    var html =
        ...
        '<p style="color:rgba(255,255,255,.8);margin:6px 0 0">' + todayQuotes.length + ' quote' + (todayQuotes.length > 1 ? 's' : '') + ' today</p>' +
        '</div>' +
        '<div style="background:#fff;padding:28px;border:1px solid #e4f1f5;border-top:none;border-radius:0 0 12px 12px">' +
...
console.log('📧 Firebase proxy URL:', FIREBASE_EMAIL_URL);
